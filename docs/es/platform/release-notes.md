...
| **Channels** | • Corrige las URLs de las redirecciones, que no quedaban codificadas y por eso no hacían match cuando el path incluía tildes u otros caracteres especiales<br>• Corrige el guardado de un sitio con configuración de host inválida, ahora la validación de dominios se aplica solo cuando el cambio afecta al host, al dominio principal o a los dominios alternativos<br>• Elimina el soporte legado de content views en el endpoint de actualización de templates de la API administrativa, que reescribía el path del template a un patrón obsoleto, las actualizaciones siguen disponibles mediante el endpoint que usa el Template Builder |
