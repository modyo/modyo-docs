...
The Modyo Platform, through a decentralized integration scheme based on APIs and microservices, is adaptable to different challenges. Unlike monolithic platforms that seek to have an internal answer to every problem, the [Modyo platform](https://es.modyo.com/plataforma) privileges integration with specialized systems instead of offering all the functionalities. This results in lighter and more adaptable platforms for every need, allowing organizations to choose within the ecosystem the best solutions that adapt to their specific realities. All this results in lower total operating and infrastructure costs by scaling only the resources that require it.
...
- Integration of administrator user accounts
...
Web sites and applications built on Modyo can be natively integrated into a large number of systems using mainly HTML and Javascript standards, through the use of the Modyo Channels template editor, which offers complete access and control of the source code of the sites managed on it.
...
By using specific Liquid tags, Modyo Channels is able to interpolate context information to tracking events, such as username, email, unique identifiers, segments to which the user belongs, among many others.
...
Through access to the source code of the pages and Widgets that make up the Web site or application, Modyo Channels allows the inclusion of special brands in each HTML component that make the use of this type of digital marketing tools more efficient.
...
Security and performance tools are implemented very similarly to analytics and product management tools, but unlike these, the focus is on capturing the experience in terms of performance, detecting Javascript errors in the browser, and service level compliance. In this category, we can mention tools such as:
- NewRelic Browser
- Dynatrace RUM
- Elastic Observability

### Integration with public APIs
Integrations with external public APIs are done directly from the Modyo Channels frontend, through the use of Widgets developed in Javascript that have the functionality of invoking content and executing actions through the use of the HTTP protocol.

Integration through APIs from Widgets requires endpoints that are publicly exposed to the Internet (or Intranet in case of private deployments). Depending on the security policies configured on the endpoint, a domain validation by CORS may be required.
