...
For them, the “Submit for Review” and “Review” buttons have an option in the drop-down menu called “Force Approval”, which allows them to see the item in the publication view.
...
:: :tip Tip
If you click Post on a particular article (for example, publish a page), the Review and Publish window will open with that selected item to see the pending changes.
::
...
:: :warning Attention
By default, Modyo saves the last 20 published `(MAX_BACKUPS)` versions of each item, so when you make the twenty-first publication, you will be deleting the backup of the item's first publication.
...
When you schedule a publication, the editable version goes into a scheduled state. When the time comes for publication, that programmed version takes the place of the version that is currently published. This way, you can schedule a change in the future to the elements, and continue working on the editable version.
...
In addition to scheduling the publication, you can also schedule the unpublication. When the time comes, the version that is published will become a backup, leaving the item in question without a published version.
...
Locks is a Modyo feature that allows you to modify a resource in a secure way, avoiding concurrency problems that may occur when another user is working on it simultaneously. This way, Modyo avoids losing work or overwriting documents.
