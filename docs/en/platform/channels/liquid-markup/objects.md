...
| **origination.uid** | Origination UID. | ```my-origination``` |
...
| **origination.can_begin** | User can start the origination. | ```true``` |
| **origination.can_create** | User can create a new submission. | ```true``` |
| **origination.can_resume** | User can resume a pending submission. | ```true``` |
| **origination.begin_message** | Message before starting. | ```"Welcome!"``` |
| **origination.completion_message** | Message on completion. | ```"All done"``` |
| **origination.ordered_steps** | Steps ordered for execution. | ```[{"uid":"step-01"}]``` |
| **origination.new_submission_url** | URL to start a submission. | ```/originations/my-origination/new``` |
...
| **submission.QUESTION_ID** | Direct access by question ID (e.g. submission.123456). | ```{"question": {"label": "What's your name?"},"text_field": "John Doe"}``` |
...
| **submission.resume_url** | URL to resume a pending submission. | ```/originations/my-origination/submissions/uuid/resume``` |
| **submission.start_info** | Metadata about start conditions. | ```{"started_by":"user","at":"2025-02-15"}``` |
| **submission.steps** | Array of step objects for the submission. | ```[{"uid":"step-01"}]``` |
| **submission.tasks** | Array of task response objects. | ```[{"task_id":"t1"}]``` |

### Task Response Types

| Object | Description | Example |
|---|---|---|
| **pending_review_task_response.approved** | Indicates if a review task was approved. | ```true``` |
| **code_snippet_task_response.data** | Data payload of a code snippet task. | ```{"language":"rb","snippet":"puts 'Hi'"}``` |
| **code_snippet_task_response.completed** | Indicates completion of code snippet task. | ```false``` |
| **user_input_task_response.fields** | Collected fields from user input task. | ```[{"label":"Name","value":"Jane"}]``` |
| **validation_task_response.validated** | Indicates validation outcome. | ```true``` |
| **digital_signature_task_response.signed** | Indicates if digital signature completed. | ```false``` |
| **origination_flow_task_response.submission** | Submission object for the flow task. |  |
