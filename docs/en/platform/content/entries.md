...
- **Translation**: This filter is associated to the selected language and filters by the translation status of the entries for the given language. Ex: This means that if an entry does not have a version in the selected language, it will be set to "Not translated".
