...
:::tip
Tip Bulk actions are executed in the background and you may not see the changes immediately, we recommend waiting a moment and refreshing the view after executing a mass action.
:::

## Bulk edit

:::warning
Warning This option is only available if you have the index of entries filtered by a particular type and you also have entries selected for bulk actions.
...
:::warning
Warning By using bulk editing of entries you are overwriting the values for the selected fields in all selected entries, so the values that existed before the bulk edit will no longer be available in the editable version.
...
:::warning
Attention If the name field is empty, the platform defaults the UUID of the entry as a slug.
:::
...
:::tip
Tip To translate an already written entry into a different language, keep in mind that your space must be configured for the languages you want to select. (Check \[Configure a Space\](/es/platform/content/spaces.html #configurar -a-space))
...
::: danger
Warning If the Space is not associated with a site, you can only see the JSON code of the entry you are developing.
:::
...
::: tip
Tip When the category title is entered, the path field will default to matching the same letters, in a url-accepted format. To change it, just edit the field.
...
::: danger
Danger If you delete a Parent Category, you also delete all its subcategories. Entries associated with these categories will lose their category assignments.
:::
