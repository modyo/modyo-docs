...
::: warning Warning If the form is public, users who do not have an active session in Modyo will see three mandatory fields at the top: Name, Last name and Email. Nombre, Apellido y Correo. Once they submit the form, a user profile with those values is created and then the response is associated with that user. :::

::: warning Warning If a user does not have an active session, and fill out the form, and the email matches one of the users already registered in Modyo, then the response will be associated with the existing user and a new user will not be created. :::
...
:::warning Atención From version 9.0.8 onwards, forms no longer have file-type questions, so you won't be able to require users to upload files using Modyo forms. :::
...
::: danger Peligro Si tu formulario ya está activado y ya tienes respuestas, ten mucho cuidado al eliminar preguntas de el, ya que al hacerlo, estarás eliminando los valores de esas respuestas que los usuario ya enviaron. This action is irreversible. :::
...
::: warning Warning In order to make proper use of the form snippet, you must take into account the _CORS_ configuration of your Modyo account and the security _headers_ of where you are inserting the snippet. :::
...
::: warning Warning If you use a Modyo site with a custom domain, you must enable the option to share resource sources [CORS](/platform/core/security.htm) in the account settings. :::
