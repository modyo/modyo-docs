...
### User Management
In a private site, user management is essential and is the main difference compared to a public site. To manage users, Modyo offers a functionality called [user realms](/en/platform/customers/overview). User realms are completely independent of each other and can have their own configurations, such as the user authentication method for each realm.
