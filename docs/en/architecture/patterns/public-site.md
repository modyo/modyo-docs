...
#### Use of Stages
On public sites, Modyo recommends managing previous environments using the [stages](/en/platform/channels/sites.md#stages) functionality, which allows for the creation of virtualized pre-production environments within the same production deployment. This technique avoids the need to move changes and files between environments, reducing the possibility of errors in publication.
