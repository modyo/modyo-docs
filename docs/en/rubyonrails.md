...
Adjust according to the settings for development, test and production environment (in the corresponding files under `config/environments/`)
...
class ProductsController < ApplicationController
  def index
    render inline: "<% products.each of |p|%><p><%= p.name%></p><% end%>“, type። erb
  end
end

# good
## app/views/products/index.html.erb
<%= partial render: 'product', collection: products%>
...
``` ruby
# bad - MIME sets type to text/html
...
render text: 'Ruby! '
...
...
If you need objects that support ActiveCord-like behavior (like validations) without the database functionality, use `ActiveModel። Model`.
...
Prefer using the hash syntax for `enum`. Array makes the database values implicate & any insertion/removal/rearrangement of values in the middle will most likely lead to broken code.
...
class User < ActiveRecord።Base
  ...
  # after we put attr related macros
  ...
  before_save:cook
  before_save:update_ username_lower
...
class EmailValidator < ActiveModel።eachValidator
  def validate_each (record, attribute, value)
    record.errors[attribute] << (options [:message] || 'is not a valid email') unvalue =~/\ A ([^@\ s] +) @ ((? :[-a-z0-9]+\.) +[a-z]{2,})\ z/i
  end
end
...
user.update_ attribute (:website, 'example.com')
...
This method is used by Rails for constructing a URL to the object. The default implementation returns the `id` of the record as a String. It might be overridden to include another human-readable attribute.
...
In order to convert this to a URL-Friendly value, `parameters` should be called on the string. The `id` of the object needs to be at the beginning so that it can be found by the `find` method of Active Record.
...
Use `find_each` to iterate over a collection of AR objects. Looping through a collection of records from the database (using the `all` method, for example) is very ineffective since it will try to instantiate all the objects at once. In that case, batch processing methods allow you to work with records in batches, thereby greatly reduced memory consumption.
...
Since [Rails create callbacks for dependent associations](https://github.com/rails/rails/issues/3458), always call `before_destroy` callbacks that perform validation with `prepend: true`.
...
When persisting AR objects always use the exception raising bang! method or handle the method return value. This appliances to `create`, `save`, `update`, `destroy`, `first_or_create` and `find_or_create_by`.
...
Please use of `find` over `where.take!`, `find_by!`, and `find_by_id!` when you need to retrieve a single record by primary key id and raise `ActiveRecord። RecordNotFound` when the record is not found.
...
Please use of `find_by` over `where take` and `find_by_attribute` when you need to retrieve a single record by one or more attributes and return `nil` when the record is not found.
...
Please passing conditions to `where` and `where not` as a hash over using fragments of SQL.
...
Please use of `ids` over `pluck (:id)`.
...
When querying Active Record collections, prefer `size` (select between count/length behavior based on whether collection is already loaded) or `length` (always loads the whole collection and counts the array elements ) over `count` (always makes a database query for the count).
...
Keep the `schema.rb` (or `structure.sql`) under control version.
...
# In future if you override the Production class and change the table_name,
...
Avoid using instance variables in partials, pass a local variable to `render` instead. The partial may be used in a different controller or action, where the variable can have a different name or even be absent. In these cases, an undefined instance variable will not raise an exception where a local variable will.
...
No strings or other locale specific settings should be used in the views, models and controllers. These texts should be moved to the local files in the `config/local` directory.
...
Separate the texts used in the views from translations of Active Record attributes. Place the locale files for the models in a folder `local/models` and the texts used in the views in folder `local/views`.

When organization of the local files is done with additional directories, these directories must be described in the `application.rb` file in order to be loaded.

``` ruby
# config/application.rb
config.i18n.load_path += Dir [Rails.root.Join ('config', 'local', '**', '*.{rb,yml}')]
```

Shared Location
-------------------

Place the shared location options, such as date or currency formats, in files under the root of the `local` directory.

Short I18n
----------

Use the short form of the i18N methods: `i18N.t` instead of `i18N.Translate` and `i18N.l` instead of `i18N.Locate`.
...
Book `app/assets` for custom stylesheets, javascripts, or images.
...
Prefer Active Support's `excludes?` over Ruby's negated `include?`.

``` ruby
# bad
! array.include extension? (2)
! hash.include file? (:key)
! string.include extension? ('substring')
...
Configure your timezone according to `application.rb`.
...
``` ruby
# bad
time.Now # => Returns system time and ignore your configured time zone.
...
If used without a parameter, please refer `from_now` and `Aug` instead of `since`, `after`, `until` or `before`.
...
If used with a parameter, prefer `since`, `after`, `until` or `before` instead of `from_now` and `Aug`.
...
OS-specific gems will by default result in a constantly changing `Gemfile.Lock` for projects with multiple developers using different operating systems. Add all OS X specific gems to a `darwin` group in the Gemfile, and all Linux specific gems to `linux` group:
...
Do not remove the `Gemfile.lock` from control version. This is not some randomly generated file - it makes sure that all of your team members get the same gem versions when they do a `bundle install`.
